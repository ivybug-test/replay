...
export default function ExecutionStatePanel({
  ...
  const visible = filter === 'all' ? events : events.filter((event) => kindOf(event) === filter)
  const currentTime = events.reduce(
    (latest, event) => event.episode_elapsed_ms <= playheadMs ? Math.max(latest, event.episode_elapsed_ms) : latest,
    -1,
  )
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="space-y-2">
        {visible.map((event) => {
          const kind = kindOf(event)
          const current = event.episode_elapsed_ms === currentTime
          return (
            <article key={`${event.sequence}:${event.event}`} className={clsx('rounded-lg border bg-ink-900/50 p-3 transition-colors', current ? 'border-amber-400/60 ring-1 ring-amber-400/20' : 'border-ink-700')}>
              ...
              <details className="mt-2 border-t border-ink-800 pt-2">
