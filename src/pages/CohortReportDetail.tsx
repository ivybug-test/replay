...
import TodolistTaskReport, { isTodolistTaskDocument } from '../components/TodolistTaskReport'
...
export default function CohortReportDetail() {
  ...
  const taskDocument = report && isTodolistTaskDocument(report.document) ? report.document : null
  return <>
    ...
    {error ? <div className="p-8 text-sm text-rose-300">{error}</div> : !report ? (
    ...
    ) : <div className={taskDocument ? 'mx-auto max-w-[1440px] space-y-5 p-8' : 'mx-auto max-w-6xl space-y-5 p-8'}>
      ...
      {taskDocument ? <TodolistTaskReport document={taskDocument} /> : <article className="card overflow-hidden">
