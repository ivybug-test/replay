...
try {
  ...
  assert.deepEqual(await tree.locator('[data-step-index]').evaluateAll((rows) => rows.map((row) => row.getAttribute('data-step-index'))), ['0', '2', '4'])
  assert.equal(await page.getByRole('button', { name: 'Collapse agent main', exact: true }).count(), 1)
  ...
  assert.deepEqual(await tree.locator('[data-step-index]').evaluateAll((rows) => rows.map((row) => row.getAttribute('data-step-index'))), ['0', '1', '3', '2', '4'])
  assert.equal(await page.getByRole('button', { name: 'Collapse agent main', exact: true }).count(), 1)
  await page.locator('[data-step-index="3"]').click()
  assert.equal(await page.getByRole('button', { name: 'Collapse agent main', exact: true }).count(), 1, 'selecting a child step keeps its ancestor path open')
  ...
  assert.equal(await page.getByRole('button', { name: 'Expand agent gui_worker', exact: true }).count(), 0)
  ...
  await page.getByRole('button', { name: 'Collapse all', exact: true }).click()
  assert.equal(await tree.locator('[data-step-index]').count(), trajectory.steps.length)
  console.log('PASS long-trajectory seeking keeps toolbar visible and top-level steps visible')

  const pollsBeforeNested = polls
  child.subagent_trajectories.push(document('nested-id', 'nested_worker', [step(1, 450, 'Nested work')]))
  const nestedDeadline = Date.now() + 10000
  while (polls <= pollsBeforeNested) {
    assert.ok(Date.now() < nestedDeadline, 'nested-agent live poll did not arrive')
    await new Promise((resolve) => setTimeout(resolve, 100))
  }
  await page.getByRole('button', { name: 'Collapse all', exact: true }).click()
  assert.equal(await tree.locator('[data-step-index]').count(), trajectory.steps.length)
  assert.ok(await page.getByRole('button', { name: 'Collapse agent main', exact: true }).isVisible())
  assert.ok(await page.getByRole('button', { name: 'Expand agent gui_worker', exact: true }).isVisible())
  assert.equal(await page.getByRole('button', { name: 'Expand agent nested_worker', exact: true }).count(), 0)
  await page.getByRole('button', { name: 'Expand agent gui_worker', exact: true }).click()
  assert.equal(await tree.locator('[data-step-index]').count(), trajectory.steps.length + child.steps.length)
  const expandNested = page.getByRole('button', { name: 'Expand agent nested_worker', exact: true })
  await expandNested.waitFor()
  await expandNested.click()
  assert.equal(await tree.locator('[data-step-index]').count(), trajectory.steps.length + child.steps.length + 1)
  assert.ok((await tree.textContent()).includes('Nested work'))
  console.log('PASS collapse all keeps roots visible and expands nested agents one level at a time')
